...
aapp.get("/auth/callback", async (req, res) => {
  ...
  try {
    ...
    console.error("❌ Error al conectar con Mercado Pago:", err.response?.data || err.message || err);
    return res.status(500).send("❌ Error al conectar con Mercado Pago.");
  }
});
